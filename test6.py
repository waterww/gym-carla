...
def play_game(env, TrainNet, TargetNet, epsilon, copy_step, n):
    ...
    if -200 < rewards < 2000:
        ...
        np.savez(f'state_action_reward_ep{n}', laterals=lateral_distance, yaws=heading_error,
        action=action, reward=reward)
    '''

    # test for local computer
    np.savez(f'state_action_reward_ep{n}', laterals=lateral_distance, yaws=heading_error,
        action=selected_actions, reward=reward_list)
...
def make_video2(env, TrainNet, n):
    ...
    client.start_recorder("/data/leuven/350/vsc35061/lib1/gym-carla/video/test_%d.log" % n)
